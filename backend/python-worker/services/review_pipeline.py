...
class ReviewPipeline:
    ...
    async def process_debug(self, job_data: Dict[str, Any]) -> bool:
        ...
        Similar to review but uses debug_doctor prompts
        """
        job_id = job_data.get("job_id")
        user_id = job_data.get("user_id")
        file_name = job_data.get("file_name", "untitled")
        ...
        try:
            print(f"\n🩺 Starting Debug Pipeline for Job: {job_id}")
            ...
            if cached_result:
                await mongodb_service.update_job_status(
                    job_id, "completed",
                    results={"content": cached_result.get("content"), "cached": True}
                )
                await mongodb_service.update_job_tokens(
                    job_id,
                    cached_result.get("tokens_used", {}),
                ...
                print(f"✅ Debug job {job_id} completed (cached)")
                return True
            ...
            tokens_used = claude_result.get("tokens_used", {})
            estimated_cost = (tokens_used.get("prompt_tokens", 0) / 1000 * 0.003) + \
                           (tokens_used.get("completion_tokens", 0) / 1000 * 0.015)

            await mongodb_service.update_job_status(
                job_id, "completed",
                results={"content": claude_result.get("content"), "cached": False}
            )

            await mongodb_service.update_job_tokens(
                job_id, tokens_used, estimated_cost, cache_hit=False
            )
            ...
            elapsed = time.time() - start_time
            print(f"✅ Debug job {job_id} completed in {elapsed:.2f}s")

            return True

        except Exception as e:
            print(f"❌ Debug pipeline error for job {job_id}: {e}")
            try:
                await mongodb_service.update_job_status(job_id, "failed", error=str(e))
            except:
                pass
            return False
